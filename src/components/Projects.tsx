...
function ProjectImage({ src, alt, className }: { src: string; alt: string; className: string }) {
  ...
  if (error) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center bg-[var(--background)] border border-[var(--border-subtle)] text-[var(--text-muted)]">
        <div className="flex flex-col items-center gap-2 opacity-50">
          ...
          <span className="text-[10px] font-mono tracking-widest opacity-60 uppercase">{src}</span>
        </div>
      </div>
    );
  }
